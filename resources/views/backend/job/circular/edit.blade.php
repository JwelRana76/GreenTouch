...
<div class="card" style="width: 500px;margin:auto">
    ...
    <div class="card-body">
        <form action="{{ route('admin.circular.update',$circular->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method("PUT")
            <div class="mb-3">
                <label for="post" class="label-control">Post</label>
                <input value="{{ $circular->post }}" type="text" name="post" id="post" class="form-control" placeholder="Enter Tile" required>
                <span class="text-danger">@error('post'){{ $message }}@enderror</span>
            </div>
            <div class="mb-3">
                <label for="death_line" class="label-control">Death Line</label>
                <input type="date" class="form-control" name="death_line" id="death_line" value="{{ $circular->death_line }}" required>
